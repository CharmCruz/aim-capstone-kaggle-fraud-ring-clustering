...
Unsupervised clustering (K-Means, DBSCAN, Hierarchical) applied to the IEEE-CIS Fraud Detection dataset to test whether transactions can be grouped into fraud-ring-like clusters — without ever using the fraud label to form the groups — and whether that grouping actually concentrates real fraud above baseline.
...
**Question:** Can transactions be grouped into clusters that correspond to the same underlying client — without using the fraud label during clustering — such that the resulting clusters concentrate known fraud well above the platform's base rate?
...
1. **Problem Framing** — scoped as unsupervised clustering; the `isFraud` label is withheld during clustering and used only afterward as an external validation signal.  
...
**Headline finding:** all three methods found fraud-concentrated groups, confirming the core premise — but only DBSCAN's result survived rigorous validation. K-Means's stronger-looking in-sample result was largely an artifact of the specific sample it was fit on.
...
Full detail, methodology justification, and the bias & fairness audit are in [`Charmagne Cruz_Capstone Final Report_20260919`](./Charmagne%20Cruz_Capstone%20Final%20Report_20260919).
...
├── Final_Report.pdf    # Full write-up: all 5 steps, methodology, results, bias audit
├── requirements.txt
└── README.md
```
